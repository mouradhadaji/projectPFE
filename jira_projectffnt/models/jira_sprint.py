...
class JiraSprint(models.Model):
    ...
    def create(self, vals_list):
        if not self.env.user.has_group('jira_project.group_jira_manager'):
            raise UserError('Vous n\'avez pas le droit de créer un sprint.')
    ...
    def write(self, vals):
        if not self.env.user.has_group('jira_project.group_jira_manager'):
            raise UserError('Vous n\'avez pas le droit de modifier un sprint.')
        return super().write(vals)

    def unlink(self):
        if not self.env.user.has_group('jira_project.group_jira_director'):
            raise UserError('Seul un Director peut supprimer un sprint.')
        return super().unlink()