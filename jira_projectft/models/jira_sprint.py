...
class JiraSprint(models.Model):
    ...
    @api.depends('ticket_ids', 'ticket_ids.story_points', 'ticket_ids.state_id.is_closed')
        ...
        for sprint in self:
            ...
            sprint.completed_story_points = sum(
                sprint.ticket_ids.filtered(
                    lambda t: t.state_id.is_closed).mapped('story_points'))

    @api.depends('start_date', 'end_date')
    def _compute_duration(self):
        for sprint in self:
            if sprint.start_date and sprint.end_date:
                sprint.duration_days = (sprint.end_date - sprint.start_date).days + 1
            else:
    ...
    def write(self, vals):
        if not self.env.user.has_group('jira_project.group_jira_manager'):
            raise UserError(_('Vous n\'avez pas le droit de modifier un sprint.'))
        res = super().write(vals)
        if 'state' in vals:
            pass  # logique future ici
        return res

    def unlink(self):
        if not self.env.user.has_group('jira_project.group_jira_director'):
            raise UserError(_('Seul un Director peut supprimer un sprint.'))
        return super().unlink()