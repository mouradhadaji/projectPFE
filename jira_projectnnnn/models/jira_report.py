...
class JiraReport(models.Model):
    ...
    def _cron_send_weekly_reports(self):
        ...
        for report in reports:
            try:
                ...
                _logger.error('Error sending weekly report %s: %s', report.name, str(e))

    # ── Cron mensuel ──────────────────────────────────────────────────────────
    @api.model
    def _cron_send_monthly_reports(self):
        reports = self.search([
            ('frequency', '=', 'monthly'),
            ('active', '=', True)
        ])
        for report in reports:
            try:
                ...
                _logger.error('Error sending monthly report %s: %s', report.name, str(e))
    ...
    def _send_report(self):
        ...
        # Sujet selon fréquence
        freq_label = 'Monthly' if self.frequency == 'monthly' else 'Weekly'

        # Envoi email
        mail_values = {
            'subject': f'📊 {freq_label} Report — {self.name}',
            'body_html': body,
        ...
        # ✅ Créé une seule fois (bug corrigé — était créé 2 fois avant)
        ...
        <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto;">
            ...
            <div style="background: linear-gradient(90deg, #0D1B2A, #1B2A3B);
                ...
                <h1 style="color: #1FC7DE; margin: 0; font-size: 24px;">
                    📊 {'Monthly' if self.frequency == 'monthly' else 'Weekly'} Report
                </h1>
