...
(function() {
    ...
    // ✅ Détruit TOUJOURS l'ancien chart avant de créer un nouveau
...
function showPerfLoading(show) {
    document.getElementById('perfLoading').style.display = show ? 'block' : 'none';
    document.getElementById('perfEmpty').style.display = show ? 'none' : 'none';
    document.getElementById('perfProjectHealth').style.display = 'none';
    document.getElementById('perfTeamReport').style.display = 'none';
}

   function renderProjectHealth(data) {
    if (data.error) return alert(data.error);
    document.getElementById('perfTotalTickets').textContent = data.total_tickets || 0;
    document.getElementById('perfCompletionRate').textContent = (data.completion_rate || 0) + '%';
    document.getElementById('perfDonePoints').textContent = data.done_points || 0;
    document.getElementById('perfHealthStatus').textContent = data.health?.status || '-';
    ...
    document.getElementById('perfProjectHealth').style.display = 'block';
}
    ...
    document.getElementById('perfTeamReport').style.display = 'block';
}
})();