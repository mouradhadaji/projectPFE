...
async function initCycleTimeChart() {
    ...
    // Observer sur project_ids pour recharger quand projets changent
    const projectField = document.querySelector('[name="project_ids"]');
    if (projectField && !projectField.dataset.cycleObserved) {
        projectField.dataset.cycleObserved = 'true';
        new MutationObserver(function() {
            setTimeout(function() { loadCycleTimeData(); }, 300);
        }).observe(projectField, { childList: true, subtree: true });
    }

    await loadCycleTimeData();
}
...
async function loadCycleTimeData() {
    ...
    if (projectField) {
        const names = Array.from(projectField.querySelectorAll('.badge, .o_tag'))
            .map(function(b) { return b.textContent.trim().replace(/[×x✕]/g, '').trim(); })
            .filter(function(n) { return n && n.length > 0; });

        if (names.length) {
            try {
                const projects = await rpcCall('jira.project', 'search_read',
                    [[['name', 'in', names]], ['id', 'name']]);
                projectIds = projects.map(function(p) { return p.id; });
            } catch(e) { console.error('Cycle time project lookup error:', e); }
        }
    }
    ...
    if (!projectIds.length) {
        if (window.cycleTimeChartInstance) {
            window.cycleTimeChartInstance.destroy();
            window.cycleTimeChartInstance = null;
        }
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        ctx.font = '14px Arial';
        ...
        ['avgCycleStory','avgCycleTask','avgCycleBug','avgCycleEpic'].forEach(function(id) {
    ...
    try {
        const tickets = await rpcCall('jira.ticket', 'search_read', [
            [['project_id', 'in', projectIds],
             ['ticket_status', 'in', ['done', 'complete']],
             ['created_date', '!=', false]],
        ...
        if (!tickets.length) {
            ...
            ctx.textAlign = 'center';
            ctx.fillStyle = '#aaa';
            ctx.fillText('No completed tickets found', canvas.width / 2, canvas.height / 2);
            return;
        }

        const typeColors = {
            'story': '#4472C4', 'task': '#ED7D31',
            'bug': '#dc3545', 'epic': '#28a745', 'subtask': '#9966FF'
        };

        const points = tickets.map(function(t) {
        ...
        // Update avg stats
        ['story', 'task', 'bug', 'epic'].forEach(function(type) {
            const tp = points.filter(function(p) { return p.type === type; });
            const avg = tp.length > 0
                ? Math.round(tp.reduce(function(s, p) { return s + p.x; }, 0) / tp.length)
                : 0;
            const idMap = { story: 'avgCycleStory', task: 'avgCycleTask',
                            bug: 'avgCycleBug', epic: 'avgCycleEpic' };
            const el = document.getElementById(idMap[type]);
            if (el) el.textContent = tp.length > 0 ? avg + 'd' : '-';
        });
        ...
        const datasets = Object.entries(typeColors)
            .map(function(entry) {
                return {
                    ...
                    data: points.filter(function(p) { return p.type === entry[0]; })
                          .map(function(p) { return { x: p.x, y: p.y }; }),
                    backgroundColor: entry[1] + '88',
                    borderColor: entry[1],
                    borderWidth: 1.5,
                    pointRadius: 6,
                    pointHoverRadius: 8
        ...
        window.cycleTimeChartInstance = new Chart(canvas.getContext('2d'), {
            ...
            options: {
                responsive: true, maintainAspectRatio: false,
                scales: {
                    x: { title: { display: true, text: 'Cycle Time (days)', color: '#666' },
                         beginAtZero: true, grid: { color: '#f0f0f0' } },
                    y: { title: { display: true, text: 'Story Points', color: '#666' },
                         beginAtZero: true, grid: { color: '#f0f0f0' } }
                },
                plugins: {
                    legend: { position: 'bottom' },
                    tooltip: { callbacks: {
                        label: function(c) {
                            return c.dataset.label + ': ' + c.parsed.x + 'd — ' + c.parsed.y + ' pts';
                        }
                    }}
                }
            }
    ...
    async function initCFDChart() {
        const canvas = document.getElementById('cfdChart');
        if (!canvas) return;

        const projectField = document.querySelector('[name="project_ids"]');
        if (projectField && !projectField.dataset.cfdObserved) {
            projectField.dataset.cfdObserved = 'true';
            new MutationObserver(function() {
                setTimeout(loadCFDData, 300);
            }).observe(projectField, { childList: true, subtree: true });
        }
    ...
    async function loadCFDData() {
        ...
        if (projectField) {
            const names = Array.from(projectField.querySelectorAll('.badge, .o_tag'))
                .map(b => b.textContent.trim().replace(/[×x✕]/g, '').trim())
        ...
        if (!projectIds.length) {
            if (window.cfdChartInstance) { window.cfdChartInstance.destroy(); window.cfdChartInstance = null; }
            ctx.clearRect(0, 0, canvas.width, canvas.height);
            ctx.font = '14px Arial'; ctx.textAlign = 'center';
            ctx.fillStyle = '#aaa';
            ctx.fillText('Select projects to see the CFD', canvas.width / 2, canvas.height / 2);
            return;
        }
        ...
        try {
            const tickets = await rpcCall('jira.ticket', 'search_read', [
                [['project_id', 'in', projectIds]],
                ['name', 'ticket_status', 'write_date', 'created_date', 'story_points']
            ]);

            if (!tickets.length) {
                ctx.clearRect(0, 0, canvas.width, canvas.height);
                ctx.font = '14px Arial'; ctx.textAlign = 'center'; ctx.fillStyle = '#aaa';
                ctx.fillText('No tickets found', canvas.width / 2, canvas.height / 2);
                return;
            }
            ...
            const statuses = ['draft', 'in_progress', 'in_review', 'done', 'complete'];
            const statusLabels = { draft: 'To Do', in_progress: 'In Progress', in_review: 'In Review', done: 'Done', complete: 'Complete' };
            const statusColors = { draft: '#AFA9EC', in_progress: '#4472C4', in_review: '#ED7D31', done: '#70AD47', complete: '#28a745' };

               // Compter les tickets par statut actuel
            const statusGroups = {};
            statuses.forEach(s => {
                statusGroups[s] = tickets.filter(t => t.ticket_status === s).length;
            });
            ...
            const datasets = statuses.map(status => {
                ...
                // Distribuer progressivement sur les semaines
                const data = weeks.map((weekDate, i) => {
                    const progress = (i + 1) / weeks.length;
                    return Math.round(count * progress);
                });
                ...
                return {
                    ...
                    data: data,
                    backgroundColor: statusColors[status] + '44',
            ...
            window.cfdChartInstance = new Chart(ctx, {
                type: 'line',
                ...
                options: {
                    ...
                    scales: {
                        ...
                        y: {
                            beginAtZero: true,
                            title: { display: true, text: 'Tickets', color: '#888' },
                            grid: { color: '#f0f0f0' },
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: c => c.dataset.label + ': ' + c.parsed.y + ' tickets'
                            }
                        }
                    }
                }
    ...
    options: {
        ...
        scales: {
            ...
            y: {
                stacked: true,   // ← RETIRER stacked ici
                beginAtZero: true,
                title: { display: true, text: 'Tickets', color: '#888' },
                grid: { color: '#f0f0f0' },
                ticks: { color: '#888', stepSize: 1 }
        ...
        } catch(e) { console.error('❌ CFD error:', e); }
    }
                ...
                if (detail && detailView) {
                    ...
                    setTimeout(function() {
                       ...
                       if (detailId === 'detail_meet') initGoogleMeet();
                        if (detailId === 'detail_cfd') initCFDChart();
                    }, 300);
                }
            });
        });
